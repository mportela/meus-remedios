...
class MedicationPhotoRepositoryImpl
    @Inject
    constructor(
        ...
        override suspend fun getAll(): List<MedicationPhoto> = photoDao.getAll().map { it.toDomain() }

        override suspend fun add(photo: MedicationPhoto): Long = photoDao.insert(photo.toEntity())

        override suspend fun update(photo: MedicationPhoto) = photoDao.update(photo.toEntity())

        override suspend fun delete(photo: MedicationPhoto) = photoDao.delete(photo.toEntity())

        override suspend fun getPhotosWithoutEmbedding(): List<MedicationPhoto> = photoDao.getWithoutEmbedding().map { it.toDomain() }
    }
