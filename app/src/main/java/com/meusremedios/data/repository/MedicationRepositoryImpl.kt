...
class MedicationRepositoryImpl
    ...
    constructor(
        ...
        override fun observeAll(): Flow<List<Medication>> = medicationDao.observeAll().map { entities -> entities.map { it.toDomain() } }

        override fun observeById(id: Long): Flow<Medication?> = medicationDao.observeById(id).map { it?.toDomain() }

        override fun search(query: String): Flow<List<Medication>> =
            medicationDao.search(query).map { entities -> entities.map { it.toDomain() } }

        override suspend fun getById(id: Long): Medication? = medicationDao.getById(id)?.toDomain()

        override suspend fun add(medication: Medication): Long = medicationDao.insert(medication.toEntity())
