...
class MarkIntakeTakenUseCaseTest {
    ...
    @Test
    fun `sem log existente - insere TAKEN`() = runTest {
        useCase(dose, date)
    ...
    @Test
    fun `log TAKEN existente - deleta (toggle off)`() = runTest {
        repo.seed(
            listOf(
                IntakeLog(
                    id = 1L,
    ...
    @Test
    fun `log SKIPPED existente - atualiza para TAKEN`() = runTest {
        repo.seed(
            listOf(
                IntakeLog(
                    id = 1L,
    ...
    @Test
    fun `sobrecarga ad-hoc - insere com scheduleTimeId nulo`() = runTest {
    ...
    private suspend fun FakeIntakeLogRepository.snapshot(): List<IntakeLog> =
        observeByDate(date).first()
}
