...
class MarkIntakeSkippedUseCaseTest {
    ...
    @Test
    fun `sem log existente - insere SKIPPED`() = runTest {
        useCase(dose, date)
    ...
    @Test
    fun `log SKIPPED existente - deleta (toggle off)`() = runTest {
        repo.seed(
            listOf(
                IntakeLog(
                    id = 1L,
    ...
    @Test
    fun `log TAKEN existente - atualiza para SKIPPED e remove takenAt`() = runTest {
        repo.seed(
            listOf(
                IntakeLog(
                    id = 1L,
