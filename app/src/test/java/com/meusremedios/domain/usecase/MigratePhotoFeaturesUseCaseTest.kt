...
class MigratePhotoFeaturesUseCaseTest {
    ...
    private val fakeFeatures = PhotoFeatures(
    ...
    private fun photo(id: Long, embedding: FloatArray? = null) = MedicationPhoto(
        id = id,
        medicationId = 1L,
    ...
    @Test
    fun `fotos sem embedding sao reprocessadas e atualizadas`() = runTest {
        val repo = object : StubPhotoRepository() {
            val updated = mutableListOf<MedicationPhoto>()
            override suspend fun getPhotosWithoutEmbedding() = listOf(photo(1L), photo(2L))
            override suspend fun update(p: MedicationPhoto) { updated += p }
        }
    ...
    @Test
    fun `sem fotos elegiveis nao chama extractor`() = runTest {
        val repo = object : StubPhotoRepository() {
    ...
    @Test
    fun `falha em uma foto nao interrompe as demais`() = runTest {
        val repo = object : StubPhotoRepository() {
            val updated = mutableListOf<MedicationPhoto>()
